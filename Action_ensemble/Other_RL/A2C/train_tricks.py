...
def train(argv):
    ...
    class CustomRewardLoggerCallback(BaseCallback):
        def __init__(self, log_file, verbose=0):
            super(CustomRewardLoggerCallback, self).__init__(verbose)
        ...
        def _on_step(self) -> bool:
            reward = self.locals['rewards'][0]  # Access rewards from the callback locals
            timestep = self.num_timesteps
            # Append reward and timestep to the log file
            with open(self.log_file, "a+") as log_f:
                log_f.write(f"{timestep},{reward}\n")
            return True
    ...
    model.learn(total_timesteps=15000000, callback=custom_reward_logger)
