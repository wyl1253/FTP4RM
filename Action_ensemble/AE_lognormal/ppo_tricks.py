...
from scipy.stats import poisson, chi2, binom, beta, gamma, lognorm
import numpy as np
import random
    ...
    def choose_action(self, s, i_episode, max_episodes):
        ...
        if self.policy_dist == "Beta":
            ...
            with torch.no_grad():
                ...
                # Lognormal 
                mu = np.log(1 + 5 * (i_episode / max_episodes))  # Log-space mean
                sigma = 0.5 + 5 * (i_episode / max_episodes)  # Log-space standard deviation

                # Sampling from the Lognormal distribution
                # The 's' parameter in lognorm is sigma, scale is exp(mu).
                i_samples = lognorm.rvs(s=sigma, scale=np.exp(mu), size=10)
                i_mean = np.clip(i_samples, 1, np.exp(mu) + sigma).mean()
                i = int(i_mean)
