...
from scipy.stats import poisson, chi2, binom, beta, gamma
import numpy as np
import random
    ...
    def choose_action(self, s, i_episode, max_episodes):
        ...
        if self.policy_dist == "Beta":
            ...
            with torch.no_grad():
                ...
                # # Gamma
                shape_parameter = 1 + 5 * (i_episode / max_episodes)  # Shape parameter 'a'
                rate_parameter = 1 + 20 * (i_episode / max_episodes)  # Rate parameter 'lambda'
                scale_parameter = 1 / rate_parameter  # Scale is the inverse of the rate

                # Sampling from the Gamma distribution
                gamma_samples = gamma.rvs(a=shape_parameter, scale=scale_parameter, size=10)
                gamma_mean = np.clip(gamma_samples, 1, rate_parameter).mean()
                i = int(gamma_mean)
                ...
                # # Beta 
                # alpha = 1 + 5 * (i_episode / max_episodes)  # alpha increases faster
                # beta_param = 1 + 20 * (i_episode / max_episodes)  # beta increases, but at a slower rate
                
                # n = 10
                # i = np.clip(n*beta.rvs(alpha, beta_param), 1, alpha)
                # aa = dist.sample((int(i),))
                # aa_a = aa.cpu().data.numpy()
                # a = aa_a.mean(axis=0)
                # a = torch.tensor(a).to(device)
                ...
                # # Exponential
                # lambda_rate = 1 / (1 + 5 * (i_episode / max_episodes))  # Define the rate parameter for the Exponential distribution
                # sample = expon.rvs(scale=1/lambda_rate)  # 'scale' is the inverse of 'lambda_rate' (i.e., mean of the distribution)
                # sample = np.clip(sample, 1, np.inf)  # Ensure sample is at least 1 or higher
                # aa = dist.sample((int(sample),))
                # aa_a = aa.cpu().data.numpy()
                # a = aa_a.mean(axis=0)
                # a = torch.tensor(a).to(device)

                # # Linear
                # beta = 1 + 5*(i_episode/max_episodes)
                # i = beta
                # aa = dist.sample((int(i),))
                # aa_a = aa.cpu().data.numpy()
                # a = aa_a.mean(axis=0)
...
##################################3
    ...
    def update(self, replay_buffer, total_steps):
        ...
        self.gamma = torch.clamp(new_gamma, 0.55, 0.99).to(device)
