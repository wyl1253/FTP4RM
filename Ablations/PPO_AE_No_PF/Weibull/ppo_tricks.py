...
from scipy.stats import poisson, chi2, binom, beta, gamma, lognorm, weibull_min
import numpy as np
import random
    ...
    def choose_action(self, s, i_episode, max_episodes):
        ...
        if self.policy_dist == "Beta":
            ...
            with torch.no_grad():
                ...
                # Weibull
                k = 1 + 5 * (i_episode / max_episodes)  # Shape parameter
                lambda_scale = 1 + 20 * (i_episode / max_episodes)  # Scale parameter
                i_samples = weibull_min.rvs(c=k, scale=lambda_scale, size=10)
                i_mean = np.clip(i_samples, 1, lambda_scale).mean()  # Adjust clipping as needed
                i = int(i_mean)
...
##################################3
    ...
    def update(self, replay_buffer, total_steps):
        ...
        self.gamma = torch.clamp(new_gamma, 0.99, 0.99).to(device)
