    ...
    def choose_action(self, s, i_episode, max_episodes):
        ...
        if self.policy_dist == "Beta":
            ...
            with torch.no_grad():
                ...
                # # Erlang
                k = 1 + int(5 * (i_episode / max_episodes))  # Ensuring k is an integer
                lambda_rate = 1 + 20 * (i_episode / max_episodes)  # Rate parameter lambda
                i_samples = gamma.rvs(a=k, scale=1/lambda_rate, size=10)
                i_mean = np.clip(i_samples, 1, lambda_rate).mean()
                i = int(i_mean)
