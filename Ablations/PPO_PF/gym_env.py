...
class ur5eGymEnv(gym.Env):
    ...
    def oxyztorus(self, x, y, z):
        ...
        while circle == False:
            ...
            b = random.random() 
            c = random.random()
            if ((a-x)**2+(b-y)**2+(c-z)**2 >= 0.09) and (a**2 + b**2 + c**2 <= 0.25) and (a**2 + b**2 >= 0.04):
    ...
    def step(self, action, i_episode, max_episodes, lp):
        ...
        self.getExtendedObservation()
        reward = self.compute_reward(self.obj_tool_pos, self.obj_tool_ori, i_episode, max_episodes, lp, joint_angles, None)
    ...
    def compute_reward(self, obj_tool_pos, obj_tool_ori, i_episode, max_episodes, lp, joint_angles, info):
        reward = 0
        ...
        # task 0: reach object:
        if errors < lp:# and approach_velocity < 0.05:
