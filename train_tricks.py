...
def train(argv):
    ...
    max_episodes = int(1e4) #int(1.5e4)
    ...
    for i_episode in range(1, max_episodes+1):
        ...
        for t in range(1, max_ep_len + 1):
            ...
            # In terms of the definition of action items, we need to integrate over the velocity then get the position to publish
            at.append(time.time())
            actions_0.append(action[0])
            actions_1.append(action[1])
            actions_2.append(action[2])
            actions_3.append(action[3])
            actions_4.append(action[4])
            actions_5.append(action[5])

            # if t > 1:
            #     action[0] += (actions_0[t-1] + actions_0[t-2]) * 0.5 * (at[t-1] - at[t-2])
            #     action[1] += (actions_1[t-1] + actions_1[t-2]) * 0.5 * (at[t-1] - at[t-2])
            #     action[2] += (actions_2[t-1] + actions_2[t-2]) * 0.5 * (at[t-1] - at[t-2])
            #     action[3] += (actions_3[t-1] + actions_3[t-2]) * 0.5 * (at[t-1] - at[t-2])
            #     action[4] += (actions_4[t-1] + actions_4[t-2]) * 0.5 * (at[t-1] - at[t-2])
            #     action[5] += (actions_5[t-1] + actions_5[t-2]) * 0.5 * (at[t-1] - at[t-2])
            ...
            # action[0] = 0 + 3.14*((action[0]+3.14)/6.28)
            # action[1] = -2.0+2.21*((action[1]+3.14)/6.28)
            # if (action[1] <= 0.25) and (action[1] >= -1):
            #     action[2] = -2.2+2.1*((action[2]+3.14)/6.28)
            # else:
            #     action[2] = -0.2+2.4*((action[2]+3.14)/6.28)
            # action[3] = -3.14+0.34*((action[3]+3.14)/6.28)
            # action[4] = -1.57+3.14*((action[4]+3.14)/6.28)
            # action[5] = -1.57+3.14*((action[4]+3.14)/6.28)
            ...
            # if (action[0]<0):
            #     action[0] = -action[0]
            # action[0] = np.clip(action[0], 0, 3.14)
            # if action[1] > 0:
            #     action[1] = -action[1]
            # elif action[1] < -2:
            #     action[1] = action[1] + 1.14
            # action[1] = np.clip(action[1], -2.0, 0)
            # if (action[1] <= 0.25) and (action[1] >= -1):
            #     action[2] = np.clip(action[2], -2.2, -0.1)
            # else:
            #     action[2] = np.clip(action[2], -0.2, 2.2)
            # if (action[3]>0):
            #     action[3] = -action[3]
            # action[3] = np.clip(action[3], -4.0, 0)
            # if (action[4]>0):
            #     action[4] = -action[4]
            # action[4] = np.clip(action[4], -2.57, -0.47)
            # action[5] = np.clip(action[5], -3.14, 3.14)
            ...
            #     if (action[2]>0.1):
            #         action[2] = -action[2]
            #     action[2] = np.clip(action[2], -2.4, 0.1)
            # else:
            #     if (action[2]<-0.2):
            #         action[2] = -action[2]
            #     action[2] = np.clip(action[2], -0.2, 2.7)
            # if (action[3]>0):
            #     action[3] = -action[3]
            # action[3] = np.clip(action[3], -4.0, 0)
            # if (action[4]>0):
